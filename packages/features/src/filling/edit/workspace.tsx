...
export function ManualEditorWorkspace({
  ...
  const { rotationSnapAngles, getSnappedRotation, buildRotationGuideLine, snapRectPosition } = useTransformGuides({
    rotationStep: 45,
    rotationTolerance: 4,
    positionTolerance: 8,
  })
  ...
  const handlePreviewWheel = useCallback((event: WheelEvent) => {
    ...
    if (previewInteractionMode === "pan") {
      ...
      if (event.shiftKey) setPreviewPan((current) => ({ ...current, x: current.x - delta }))
  ...
  }, [canvasHeight, canvasWidth, clampPreviewZoom, fitScale, previewInteractionMode, previewPan.x, previewPan.y, previewZoom, stageSize.height, stageSize.width])
    ...
    if (hasDraggedSelectionBox) {
      ...
      const selectedIds = layers
        ...
        .filter((layer) => {
          ...
          return bounds.x >= selectionBoxRect.x &&
  ...
  const handleDragMove = useCallback((layerId: string, e: Konva.KonvaEventObject<DragEvent>) => {
    const node = e.target
    const layer = layers.find((candidate) => candidate.id === layerId)
    if (!layer) return
    const draftLayer: VectorLayer = { ...layer, x: (node.x() - offsetX) / renderScale, y: (node.y() - offsetY) / renderScale }
    ...
    const { snappedRect, guides } = snapRectPosition({
      ...
      canvasRect: { x: 0, y: 0, width: canvasWidth, height: canvasHeight } as RectBounds,
    })
  ...
  }, [canvasHeight, canvasWidth, layers, offsetX, offsetY, renderScale, snapRectPosition])
  ...
  const handleTransform = useCallback((e: Konva.KonvaEventObject<Event>) => {
    ...
    setRotationGuideLine(buildRotationGuideLine(centerX, centerY, snappedRotation.snapAngle ?? snappedRotation.rotation, guideLength))
  ...
  const groupConnectionOverlays = useMemo(() => groups.flatMap((group) => buildGroupOverlayPolygons(group, layers)), [groups, layers])
      ...
      <div ref={containerRef} className="relative w-full bg-slate-100 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden" style={{ height: `${previewContainerHeight}px`, cursor }}>
        <Stage ref={stageRef} width={stageSize.width} height={stageSize.height} onClick={handleStageClick} onTap={handleStageClick} onMouseDown={handleStageMouseDown} onMouseUp={handleStageMouseUp} onMouseMove={(e) => { handleStageMouseMove(e); const targetName = e.target.name(); if (targetName.includes("rotater")) { setCursor("crosshair"); return } if (targetName.includes("manual-layer-shape")) { const isPointerDown = (e.evt as MouseEvent).buttons === 1; setCursor(isPointerDown ? "grabbing" : "grab"); return } setCursor("default") }} onMouseLeave={() => setCursor("default")}>
          <Layer>
            ...
            {rotationGuideLine ? <Line key="manual-rotation-guide" points={rotationGuideLine} stroke="rgba(14, 165, 233, 0.9)" strokeWidth={1.5} dash={[10, 6]} listening={false} perfectDrawEnabled={false} /> : null}
            {groupConnectionOverlays.map((overlay) => {
              const scaledPoints = flattenPoints(overlay.points).map((value, index) => value * renderScale + (index % 2 === 0 ? offsetX : offsetY))
              const isInterior = overlay.type === "interior"
              const isCombinedHull = overlay.type === "combined-hull"
              return <Line key={overlay.id} name="manual-group-overlay" points={scaledPoints} closed fill={isCombinedHull ? "rgba(245, 158, 11, 0.18)" : isInterior ? "rgba(250, 204, 21, 0.28)" : "rgba(250, 204, 21, 0.06)"} stroke={isCombinedHull ? "rgba(194, 65, 12, 0.95)" : isInterior ? "rgba(217, 119, 6, 0.95)" : "rgba(217, 119, 6, 0.82)"} strokeWidth={isCombinedHull ? 2 : isInterior ? 1.8 : 1.4} dash={isInterior || isCombinedHull ? undefined : [6, 4]} listening={false} perfectDrawEnabled={false} />
            })}
            {layers.map((layer) => {
              if (!layer.visible) return null
              const flat = flattenPoints(resolveLayerShapePoints(layer))
              const scaledFlat = flat.map((value) => value * renderScale)
              return <Line key={layer.id} id={`layer-${layer.id}`} name="manual-layer-shape" points={scaledFlat} x={offsetX + layer.x * renderScale} y={offsetY + layer.y * renderScale} rotation={layer.rotation} closed fill="rgba(59, 130, 246, 0.15)" stroke={selectedLayerIds.includes(layer.id) ? "#3b82f6" : "#94a3b8"} strokeWidth={selectedLayerIds.includes(layer.id) ? 2 : 1} draggable={!layer.locked} onClick={(event) => { if (event.evt.ctrlKey || event.evt.metaKey) { onToggleLayerSelection(layer.id); return } onSelectLayer(layer.id) }} onTap={() => onSelectLayer(layer.id)} onMouseEnter={() => setCursor(layer.locked ? "not-allowed" : "grab")} onMouseLeave={() => setCursor("default")} onDragStart={() => { setPositionGuideLines([]); setRotationGuideLine(null); setCursor("grabbing") }} onDragMove={(e) => handleDragMove(layer.id, e)} onDragEnd={(e) => handleDragEnd(layer.id, e)} onTransformStart={() => { setPositionGuideLines([]); setRotationGuideLine(null); setCursor("grabbing") }} onTransform={handleTransform} onTransformEnd={(e) => handleTransformEnd(layer.id, e)} />
            })}
            {selectionBoxRect ? <Rect x={offsetX + selectionBoxRect.x * renderScale} y={offsetY + selectionBoxRect.y * renderScale} width={selectionBoxRect.width * renderScale} height={selectionBoxRect.height * renderScale} fill="rgba(14, 165, 233, 0.12)" stroke="rgba(14, 165, 233, 0.85)" strokeWidth={1} dash={[6, 4]} listening={false} /> : null}
            <Transformer ref={transformerRef} rotateEnabled keepRatio={!isFreeAspectRatio} rotationSnaps={rotationSnapAngles} rotationSnapTolerance={4} enabledAnchors={["top-left", "top-right", "bottom-left", "bottom-right"]} boundBoxFunc={(oldBox, newBox) => (Math.abs(newBox.width) < 10 || Math.abs(newBox.height) < 10 ? oldBox : newBox)} />
          </Layer>
        </Stage>
