...
} from "@imify/features/filling/filling-export-worker-protocol"
...
const workerPostMessage = globalThis as unknown as { postMessage: (message: FillExportWorkerResponseMessage, transfer?: Transferable[]) => void }
const postProgress = (id: number, percent: number, message: string) => workerPostMessage.postMessage({ id, type: "progress", percent: Math.max(0, Math.min(100, Math.round(percent))), message })
const toErrorMessage = (error: unknown) => (error instanceof Error && error.message ? error.message : "Fill export failed in worker")
...
async function handleExport(message: FillExportWorkerRequestMessage): Promise<void> {
  ...
  try {
    ...
    loadedImages = await loadAllImagesAsBitmaps(payload.layerFillStates, (completed, total) => {
      if (total === 0) return postProgress(id, 20, "No layer images to load")
    ...
    const imageData = await renderFilledCanvas({ template: payload.template, layerFillStates: payload.layerFillStates, canvasFillState: payload.canvasFillState, loadedImages, backgroundImage: backgroundBitmap, runtimeItems: payload.runtimeItems, groupRuntimeTransforms: payload.groupRuntimeTransforms })
    postProgress(id, 78, `Encoding ${payload.targetFormat.toUpperCase()}...`)
    const outputBlob = await encodeFilledImageData({ imageData, targetFormat: payload.targetFormat, quality: payload.quality, formatOptions: payload.formatOptions })
    postProgress(id, 95, "Finalizing file...")
    const outputBuffer = await outputBlob.arrayBuffer()
    workerPostMessage.postMessage({ id, type: "result", ok: true, outputBuffer, mimeType: outputBlob.type || `image/${payload.targetFormat}` }, [outputBuffer])
  } catch (error) {
    workerPostMessage.postMessage({ id, type: "result", ok: false, error: toErrorMessage(error) })
  } finally {
    if (backgroundBitmap) backgroundBitmap.close()
    for (const bitmap of loadedImages.values()) bitmap.close()
  }
}
