...
function findBestAxisDelta(movingAnchors: number[], targetAnchors: number[], tolerance: number): { delta: number; targetValue: number } | null {
...
export function useTransformGuides(options: UseTransformGuidesOptions = {}) {
  ...
  const rotationSnapAngles = useMemo(() => createRotationSnapAngles(rotationStep), [rotationStep])

  const getSnappedRotation = useCallback((rotation: number, tolerance = rotationTolerance): RotationSnapResult => {
    ...
    if (bestDelta === null || bestAngle === null || Math.abs(bestDelta) > tolerance) {
      return { snapped: false, rotation, snapAngle: null }
    }

    return { snapped: true, rotation: rotation + bestDelta, snapAngle: bestAngle }
  }, [rotationSnapAngles, rotationTolerance])

  const buildRotationGuideLine = useCallback((centerX: number, centerY: number, angleDeg: number, length: number): number[] => {
  ...
  const snapRectPosition = useCallback(({
    ...
    if (bestX) guides.push({ orientation: "vertical", value: bestX.targetValue })
    if (bestY) guides.push({ orientation: "horizontal", value: bestY.targetValue })

    return { snappedRect, guides }
  }, [positionTolerance])

  return { rotationSnapAngles, getSnappedRotation, buildRotationGuideLine, snapRectPosition }
}
