...
export function SymmetricWorkspace({
  template,
  onRefresh,
  onSaved,
}: SymmetricWorkspaceProps) {
  ...
  const [selectedControlId, setSelectedControlId] =
    useState<SymmetricControlId | null>(null);
  ...
  const setSymmetricLayerCount = useFillingStore(
    (state) => state.setSymmetricLayerCount,
  );
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      setSymmetricParams((previous) => {
        ...
        if (selectedControlId === SECOND_CONTROL_ID) {
          ...
          return {
            ...previous,
            shapeSpacing: Math.max(
              0,
              previous.shapeSpacing + metrics.shapeOrderSign * appearanceDelta,
            ),
          };
        }
        ...
        return {
          ...
          axisSpacing: Math.max(
            0,
            previous.axisSpacing + metrics.axisOrderSign * axisDelta,
          ),
        };
      });
  ...
  const selectedControl = selectedControlId
    ? controlMap.get(selectedControlId) ?? null
    : null;
  ...
  const applyFirstControlRect = useCallback(
    (worldRect: WorldRect, includeResize: boolean) => {
      setSymmetricParams((previous) => {
        const metrics = deriveSymmetricLayoutMetrics(previous);
        const next: SymmetricParams = { ...previous };

        if (metrics.isHorizontal) {
          next.firstShapePosition = Math.round(
            worldRect.x - metrics.parallelMinAdjust,
          );
          next.firstAxisPosition = Math.round(worldRect.y);
        } else {
          next.firstShapePosition = Math.round(
            worldRect.y - metrics.parallelMinAdjust,
          );
          next.firstAxisPosition = Math.round(worldRect.x);
        }

        if (includeResize) {
          const newSideLength = metrics.isHorizontal
            ? worldRect.height
            : worldRect.width;
          const newParallelSpan = metrics.isHorizontal
            ? worldRect.width
            : worldRect.height;
          const parallelScale =
            newParallelSpan / Math.max(1, metrics.parallelSpan);

          next.sideLength = Math.max(1, Math.round(newSideLength));
          next.baseLength = scaleLength(metrics.baseLength, parallelScale);
          next.oppositeBaseLength = scaleLength(
  ...
  const applySecondControlRect = useCallback(
    (worldRect: WorldRect) => {
      setSymmetricParams((previous) => {
        const metrics = deriveSymmetricLayoutMetrics(previous);
        const draggedShapeStart =
          (metrics.isHorizontal ? worldRect.x : worldRect.y) -
          metrics.parallelMinAdjust;
        const signedStride =
          metrics.shapeOrderSign *
          (draggedShapeStart - metrics.firstShapePosition);

        return {
          ...previous,
          shapeSpacing: Math.max(
            0,
            Math.round(signedStride - metrics.parallelSpan),
          ),
        };
      });
    },
  ...
  const applyThirdControlRect = useCallback(
    (worldRect: WorldRect) => {
      setSymmetricParams((previous) => {
        const metrics = deriveSymmetricLayoutMetrics(previous);
        const draggedShapeStart =
          (metrics.isHorizontal ? worldRect.x : worldRect.y) -
          metrics.parallelMinAdjust;
        const draggedAxisOffset = metrics.isHorizontal
          ? worldRect.y
          : worldRect.x;
        const signedAxisStride =
          metrics.axisOrderSign *
          (draggedAxisOffset - metrics.firstAxisPosition);

        return {
          ...previous,
          oddEvenOffset: Math.round(
            draggedShapeStart - metrics.firstShapePosition,
          ),
          axisSpacing: Math.max(
            0,
            Math.round(signedAxisStride - metrics.sideLength),
          ),
        };
      });
    },
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            <Subheading>{t("dialog.symmetricTitle")}</Subheading>
            <VisualHelpTooltip
              label={t("tooltips.symmetricHelpLabel")}
              description={t("tooltips.symmetricHelpDesc")}
              webmSrc={resolveFeatureMediaAssetUrl(
                FEATURE_MEDIA_ASSETS.filling.symmetricVisualEditorWebm,
              )}
              buttonAriaLabel={t("tooltips.symmetricHelpAria")}
              mediaAlt={t("tooltips.symmetricHelpMediaAlt")}
            />
          </div>
          <MutedText className="mt-0.5 text-xs truncate">
            {generatedLayers.length === 1
              ? t("symmetric.shapesGenerated", { count: 1 })
              : t("symmetric.shapesGeneratedPlural", {
                  count: generatedLayers.length,
                })}{" "}
        ...
        >
          <Layer>
            ...
            {generatedLayers.map((layer) => {
              const flat = flattenPoints(layer.points).map(
            ...
            {controls.map((control) => {
              ...
              const stageWidth = Math.max(
                8,
                control.bounds.width * renderScale,
              );
              const stageHeight = Math.max(
                8,
                control.bounds.height * renderScale,
              );
              ...
              return (
                <Rect
                  ...
                  onDragMove={(event) => {
                    const movingWorldRect = toWorldRect(
                    ...
                    const stageGuides = guides.map((guide) => {
                      if (guide.orientation === "vertical") {
                        ...
                        return [
                          ...
                          offsetY,
                          x,
                          offsetY + template.canvasHeight * renderScale,
                        ];
                      }
                      ...
                      return [
                        offsetX,
                        y,
                        offsetX + template.canvasWidth * renderScale,
                        y,
                      ];
                    });
