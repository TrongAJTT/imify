...
export function GroupLayerPanel({
  ...
  return (
    <AccordionCard
      ...
      sublabel={
        members.length === 1
          ? t("manualEditor.groupMemberSingular", {
              count: 1,
              defaultValue: "1 layer",
            })
          : t("manualEditor.groupMemberPlural", {
              count: members.length,
              defaultValue: `${members.length} layers`,
            })
      }
      colorTheme="orange"
      defaultOpen={true}
    >
      <div className="space-y-3">
        <div className="flex gap-2 items-end">
          <TextInput
            label={t("manualEditor.groupNameLabel", {
              defaultValue: "Group Name",
            })}
            value={group.name}
            onChange={onRenameGroup}
            placeholder={t("manualEditor.groupNamePlaceholder", {
              defaultValue: "Group name",
            })}
            className="flex-1"
          />
          ...
          <Button
            variant="secondary"
            size="sm"
            onClick={onUngroupSelectedLayer}
        ...
        <CheckboxCard
          title={t("manualEditor.combineHullTitle", {
            defaultValue: "Combine into one convex hull",
          })}
          subtitle={t("manualEditor.combineHullDesc", {
            defaultValue:
              "Creates one merged hull from all members (includes close loop + fill interior behavior).",
          })}
        ...
        <CheckboxCard
          title={t("manualEditor.closeLoopTitle", {
            defaultValue: "Close loop (connect last to first)",
          })}
          subtitle={t("manualEditor.closeLoopDesc", {
            defaultValue:
              "Adds a connector hull between the last and first layer.",
          })}
        ...
        <CheckboxCard
          title={t("manualEditor.fillInteriorTitle", {
            defaultValue: "Fill interior (solid enclosed area)",
          })}
          subtitle={t("manualEditor.fillInteriorDesc", {
            defaultValue:
              "Fills enclosed regions created by connector hull intersections.",
          })}
