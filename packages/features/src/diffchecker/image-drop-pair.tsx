...
function DropZone({ label, image, onLoad, onClear }: { label: string; image: DiffImageItem | null; onLoad: (files: File[]) => void; onClear: () => void }) {
  ...
  const displayLabel = label === "Image A" ? t("imageA") : t("imageB")
  ...
  if (image) {
    return (
      <div className="relative min-w-0 flex-1 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/30">
        <button type="button" onClick={onClear} className="absolute top-2 right-2 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-slate-900/60 text-white" aria-label="Clear image"><X size={12} /></button>
        <div className="absolute top-9 right-2 z-10">
          <Tooltip content={t("tooltips.replaceImage", { label: displayLabel })} variant="nowrap">
            <button type="button" onClick={() => openFilePicker(onLoad)} className="flex h-6 w-6 items-center justify-center rounded-full bg-slate-900/60 text-white" aria-label="Replace image"><Upload size={13} /></button>
          </Tooltip>
        </div>
        <div className="flex items-center gap-3">
          <img src={image.url} alt={displayLabel} className="h-16 w-16 shrink-0 rounded object-cover border border-slate-200 dark:border-slate-700" />
          <div className="min-w-0 flex-1">
            <span className="text-[10px] font-bold uppercase tracking-wider text-sky-500">{displayLabel}</span>
  ...
  return (
    <div className="flex min-w-0 flex-1 cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 py-10 transition-all hover:border-sky-400 dark:border-slate-600 dark:bg-slate-800/30" onClick={() => openFilePicker(onLoad)} onDragOver={(e) => { if (hasFileDragPayload(e.dataTransfer)) e.preventDefault() }} onDrop={handleDrop}>
      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{displayLabel}</span>
      ...
      <MutedText className="text-xs">{t("dropOrClickBrowse")}</MutedText>
    </div>
  )
}
