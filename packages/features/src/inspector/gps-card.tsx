...
export function GpsCard({ gps }: { gps: GpsInfo }) {
  ...
  return (
    <InfoSection title={t("gpsLocation")} icon={<MapPin size={13} />}>
      <div className="mb-3 overflow-hidden rounded-lg border border-red-200 bg-red-50 px-3 py-2 dark:border-red-800 dark:bg-red-900/20">
        <div className="flex items-start gap-2"><MapPin size={14} className="mt-0.5 flex-shrink-0 text-red-500" /><span className="text-xs font-medium text-red-700 dark:text-red-300">{t("gpsWarning")}</span></div>
      </div>
      <div className="mt-3 divide-y divide-slate-100 dark:divide-slate-700/50">
        <InfoRow label={t("latitude")} value={gps.latitudeDms} mono />
        <InfoRow label={t("longitude")} value={gps.longitudeDms} mono />
        <InfoRow label={t("decimal")} value={`${gps.latitude.toFixed(6)},${gps.longitude.toFixed(6)}`} mono />
        {gps.altitude !== null ? <InfoRow label={t("altitude")} value={`${gps.altitude.toFixed(1)} m`} /> : null}
      </div>
