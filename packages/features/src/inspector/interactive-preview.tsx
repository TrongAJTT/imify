...
type CanvasPointingEvent = React.PointerEvent<HTMLCanvasElement> | React.MouseEvent<HTMLCanvasElement>
...
export function InteractivePreview({
    ...
    image.onload = () => {
      ...
      for (let index = 0; index < transformed.length; index += 4) {
        const [r, g, b, a] = transformPixelForPreview(
          transformed[index], transformed[index + 1], transformed[index + 2], transformed[index + 3], channelMode, colorBlindMode
        )
        transformed[index] = r
  ...
  useEffect(() => {
    if (!sample || !loupeEnabled) return
    const previewCanvas = previewCanvasRef.current
    const loupeCanvas = loupeCanvasRef.current
    if (!previewCanvas || !loupeCanvas) return
  ...
  const readSampleFromPointer = (event: CanvasPointingEvent): PixelSample | null => {
    ...
    if (!canvas || !transformed) return null
    ...
    return { x, y, r, g, b, a, hex: rgbToHex(r, g, b) }
  }
  ...
  return (
    <div className="relative mb-3 flex h-full w-full min-w-0 items-center justify-center overflow-hidden" style={{ maxHeight: maxDisplayHeight }}>
      ...
      {!hideOverlays && loupeEnabled && isReady && sample ? (
        <div className="absolute right-2 top-2 rounded-md border border-slate-300/70 bg-slate-950/70 p-1 backdrop-blur-sm dark:border-slate-700/70">
          <canvas ref={loupeCanvasRef} style={{ imageRendering: "pixelated" }} className="rounded" />
          <div className="mt-1 text-center text-[10px] text-white/90">{loupeZoom}x loupe</div>
        </div>
      ) : null}
      {!hideOverlays && sample && (
