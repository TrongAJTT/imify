...
export function DisplayAccordion(props: DisplayAccordionProps) {
  ...
  return (
    <AccordionCard icon={<Palette size={16} />} label={t("display")} sublabel={sublabel} colorTheme="blue" alwaysOpen>
      <div className="space-y-3">
        <SliderInput label={t("paletteColors")} value={props.paletteCount} onChange={props.onPaletteCountChange} min={4} max={12} step={2} />
        <SelectInput label={t("previewChannel")} value={props.previewChannelMode} options={channelOptions} onChange={(value) => props.onPreviewChannelModeChange(value as PreviewChannelMode)} />
        <SelectInput label={t("colorBlindSim")} value={props.colorBlindMode} options={colorBlindOptions} onChange={(value) => props.onColorBlindModeChange(value as ColorBlindMode)} />
        <CheckboxCard title={t("enableLoupe")} subtitle={t("loupeSubtitle")} checked={props.loupeEnabled} onChange={props.onLoupeEnabledChange} theme="blue" />
        {props.loupeEnabled ? <SliderInput label={t("loupeZoom")} value={props.loupeZoom} onChange={props.onLoupeZoomChange} min={2} max={12} step={1} suffix="x" /> : null}
      </div>
    </AccordionCard>
  )
}
