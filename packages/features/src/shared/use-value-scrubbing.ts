...
export interface UseValueScrubbingOptions {
  ...
  /**
   * Percent value change per 1px horizontal drag.
   * Increase this for faster scrubbing than wheel default step.
   */
  percentPerPx?: number

  /**
   * Additional acceleration based on total drag distance.
   * We keep it modest to avoid jumping too much.
   */
...
export function useValueScrubbing({
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent<HTMLElement>) => {
      ...
      // Avoid starting scrubbing when user is trying to drag-select / etc.
      e.preventDefault()
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent<HTMLElement>) => {
      ...
      // Acceleration: farther drag => higher sensitivity.
