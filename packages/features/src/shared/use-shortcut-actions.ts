...
export function useShortcutActions(actions: ShortcutActionHandler[], enabled: boolean = true) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      for (const action of actionsRef.current) {
        ...
        if (!binding) continue
        if (!action.allowWhenEditable && isShortcutEventFromEditableTarget(event)) continue
        if (!eventMatchesShortcut(event, binding)) continue
        if (action.preventDefault !== false) event.preventDefault()
