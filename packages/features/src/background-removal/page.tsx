...
export function SharedBackgroundRemoverPage({
  ...
  const unloadModelAfterProcess = useBackgroundRemoverStore(
    (s) => s.unloadModelAfterProcess,
  );
  ...
  const processOutput = useCallback(
    async (output: any) => {
      ...
      const exponent =
        edgeSmoothing < 0 ? 1 + Math.abs(edgeSmoothing) * 0.25 : 1;
      ...
      for (let i = 0; i < resultData.data.length; i += 4) {
        ...
        if (outputFormat === "color") {
          resultData.data[i] =
            resultData.data[i] * alpha + bgRgb.r * (1 - alpha);
          resultData.data[i + 1] =
            resultData.data[i + 1] * alpha + bgRgb.g * (1 - alpha);
          resultData.data[i + 2] =
            resultData.data[i + 2] * alpha + bgRgb.b * (1 - alpha);
  ...
  const { removeBackground, isProcessing, progressPayload } =
