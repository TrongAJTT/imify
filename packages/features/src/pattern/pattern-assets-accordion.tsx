...
type DrawingSession = { kind: "create" } | { kind: "edit"; assetId: string };
...
export function PatternAssetsAccordion() {
  ...
  const reorderAssetsByIds = usePatternStore(
    (state) => state.reorderAssetsByIds,
  );

  const fileInputRef = useRef<HTMLInputElement>(null);
  const [drawingSession, setDrawingSession] = useState<DrawingSession | null>(
    null,
  );
  ...
  const handleAssetFilesUpload = async (
    ...
    try {
      ...
      if (drawingSession?.kind === "edit") {
        const currentAsset = assets.find(
          (asset) => asset.id === drawingSession.assetId,
        );
  ...
  const sublabel =
    assets.length > 0
      ? assets.length === 1
        ? t("assetsFields.layersCount", { count: assets.length })
        : t("assetsFields.layersCountPlural", { count: assets.length })
      : t("assetsFields.noLayersYet");
  ...
  return (
    <>
        ...
        <div className="space-y-2.5">
          ...
          <div className="sticky top-0 z-10 -mx-1 border-b border-slate-200/80 bg-white/95 px-1 pb-2 pt-0.5 backdrop-blur-sm dark:border-slate-700/80 dark:bg-slate-900/95">
            <div className="flex gap-1.5">
              ...
              >
                <Brush size={13} />
                {t("assetsFields.drawBtn")}
              </Button>

              {assets.length > 0 && (
                <Tooltip content={t("tooltips.clearAllAssets")}>
                  <Button
                    className="flex h-8 px-2.5"
                    variant="secondary"
                    size="sm"
                ...
                <div className="space-y-2">
                  {assets.map((asset) => (
                    <PatternAssetListItem
                      key={asset.id}
                      asset={asset}
                      onToggleEnabled={(assetId, enabled) =>
                        updateAsset(assetId, { enabled })
                      }
                      onOpacityChange={(assetId, opacity) =>
                        updateAsset(assetId, { opacity })
                      }
                      onMonochromeChange={(assetId, monochrome) =>
                        updateAsset(assetId, { monochrome })
                      }
                      onBorderChange={(assetId, border) =>
                        updateAsset(assetId, { border })
                      }
                      onCornerRadiusChange={(assetId, cornerRadius) =>
                        updateAsset(assetId, { cornerRadius })
                      }
      ...
      <PatternAssetDrawingDialog
        ...
        initialSuggestedName={toSuggestedAssetName(
          editingAsset?.name ?? "drawn-asset",
        )}
