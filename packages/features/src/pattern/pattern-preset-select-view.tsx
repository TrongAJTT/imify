...
function sortPresets(presets: SavedPatternPreset[]): SavedPatternPreset[] {
  const sortByUpdatedAt = (a: SavedPatternPreset, b: SavedPatternPreset) =>
    b.updatedAt - a.updatedAt;
  const pinned = presets
    .filter((preset) => preset.isPinned)
    .sort(sortByUpdatedAt);
  const unpinned = presets
    .filter((preset) => !preset.isPinned)
    .sort(sortByUpdatedAt);
        ...
        <div className="mt-1.5 flex items-center justify-between gap-2 text-[11px]">
          ...
          {isActive ? (
            <span className="inline-flex items-center gap-1 font-semibold text-sky-600 dark:text-sky-400">
              <Check size={12} />
              {t("select.activeText", { defaultValue: "Active" })}
            </span>
          ) : null}
        </div>
      ...
      {sortedPresets.length === 0 ? (
        ...
        <>
          <WorkspaceSelectHeader
            title={t("select.presetsTitle")}
            createLabel={t("select.createLabel")}
