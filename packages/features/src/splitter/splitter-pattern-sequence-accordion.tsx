...
function SortablePatternCard({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-center gap-2 rounded-md border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-900"
    >
...
export function SplitterPatternSequenceAccordion({
  ...
  const showXAxis =
    settings.direction === "vertical" || settings.direction === "grid";
  const showYAxis =
    settings.direction === "horizontal" || settings.direction === "grid";
  ...
  const updateAxisPattern = (axis: AxisKey, values: number[]) => {
    ...
    if (isPercentPattern) {
      onChange(
        axis === "x"
          ? { percentPatternX: serialized }
          : { percentPatternY: serialized },
      );
      return;
    }
    onChange(
      axis === "x"
        ? { pixelPatternX: serialized }
        : { pixelPatternY: serialized },
    );
  };
  ...
  const renderAxisSection = (
    ...
    <div className="space-y-2">
      ...
      <DndContext
        ...
        onDragEnd={(event) => reorderAxis(axis, event)}
      >
        <SortableContext
  ...
  return (
    <AccordionCard
      ...
      label={
        isPercentPattern ? t("percentPatternGuides") : t("pixelPatternGuides")
      }
      sublabel={t("patternSequenceSublabel")}
      colorTheme="sky"
      isOpen={isOpen}
    ...
    >
      <div className="space-y-3">
        {showXAxis
          ? renderAxisSection("x", t("verticalGuidesSequence"), xEntries)
          : null}
        {showYAxis
          ? renderAxisSection("y", t("horizontalGuidesSequence"), yEntries)
          : null}
      </div>
    </AccordionCard>
  );
