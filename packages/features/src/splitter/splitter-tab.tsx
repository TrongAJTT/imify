...
export function SplitterTab({ onRootClick }: SplitterTabProps = {}) {
  ...
  const [previewInteractionMode, setPreviewInteractionMode] =
    useState<PreviewInteractionMode>("idle");
  const [previewPlanWarningText, setPreviewPlanWarningText] = useState<
    string | null
  >(null);
  ...
  const mismatchWarningText = useMemo(() => {
    ...
    const mismatchCount = images.filter(
      (image) =>
        image.originalWidth !== first.originalWidth ||
        image.originalHeight !== first.originalHeight,
    ).length;
  ...
  useEffect(() => {
    ...
    const computePreview = async () => {
      ...
      try {
        ...
        setPreviewPlanWarningText(
          visibleWarnings.length > 0 ? visibleWarnings.join(" ") : null,
        );
      } catch (error) {
        if (cancelled || previewComputeSequenceRef.current !== requestId) {
          return;
    ...
    async (incomingFiles: File[]) => {
      ...
      for (let index = 0; index < imageFiles.length; index += 1) {
        ...
        const percent = Math.min(
          88,
          5 + Math.round(((index + 1) / imageFiles.length) * 83),
        );
        pushImportToast({
          id: toastId,
          fileName: `Importing ${imageFiles.length} images`,
  ...
  const handleBasicGuideChange = useCallback(
    (axis: "x" | "y", sourceValue: number) => {
      ...
      const axisSize =
        axis === "x" ? activeImage.originalWidth : activeImage.originalHeight;
      if (axisSize <= 1) {
        return;
      }
      ...
      if (splitSettings.basicMethod === "count") {
        const nextCount = clamp(
          Math.round(axisSize / safeSourceValue),
          1,
          4096,
        );
        setSplitSettings(
          axis === "x" ? { countX: nextCount } : { countY: nextCount },
        );
        return;
      }
      ...
      if (splitSettings.basicMethod === "percent") {
        const nextPercent = clamp(
          Math.round((safeSourceValue / axisSize) * 100),
          1,
          100,
        );
        setSplitSettings(
          axis === "x" ? { percentX: nextPercent } : { percentY: nextPercent },
        );
        return;
      }
      ...
      setSplitSettings(
        axis === "x" ? { pixelX: nextPixels } : { pixelY: nextPixels },
      );
    },
    [
      activeImage,
      setSplitSettings,
      splitSettings.basicMethod,
      splitSettings.mode,
    ],
  );
    ...
    try {
      for (let imageIndex = 0; imageIndex < images.length; imageIndex += 1) {
        ...
        pushExportToast({
          ...
          percent: Math.min(
            70,
            5 + Math.round(((imageIndex + 0.25) / images.length) * 60),
          ),
        ...
        const convertedSegments = await convertSplitterSegments({
          ...
          onProgress: ({ completed, total }) => {
            ...
            const imageStep =
              (Math.max(1, completed) / Math.max(1, total)) *
              (60 / Math.max(1, images.length));
            const percent = Math.min(
              85,
              10 + Math.round(imageBase + imageStep),
            );
            pushExportToast({
              id: toastId,
              fileName: "Split export",
      ...
      if (downloadMode === "zip") {
        ...
        for (let index = 0; index < exportFiles.length; index += 1) {
          ...
          const percent = Math.min(
            98,
            88 +
              Math.round(((index + 1) / Math.max(1, exportFiles.length)) * 10),
          );
          pushExportToast({
            id: toastId,
            fileName: "Split export",
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                {mismatchWarningText ? (
                  <Tooltip
                    label={t("dimensionMismatchLabel")}
                    content={mismatchWarningText}
                    variant="wide2"
                  >
