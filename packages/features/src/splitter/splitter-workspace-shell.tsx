...
interface SplitterPresetSource {
  ...
  exportSettings: ReturnType<
    typeof useSplitterStore.getState
  >["exportSettings"];
}
...
export function SplitterWorkspaceShell({
  workspace,
  onRootClick,
}: SplitterWorkspaceShellProps) {
  ...
  const activePresetId = useSplitterPresetStore(
    (state) => state.activePresetId,
  );
  const defaultPresetBootstrapped = useSplitterPresetStore(
  ...
  const syncActivePresetConfig = useSplitterPresetStore(
