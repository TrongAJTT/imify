...
export function DonateDialog({ isOpen, onClose }: DonateDialogProps) {
  ...
  const buyMeCoffeeLogoSrc = resolveFeatureMediaAssetUrl(
    FEATURE_MEDIA_ASSETS.brand.buyMeCoffeeLogoSvg,
  );
  const paypalIconSvg = resolveFeatureMediaAssetUrl(
    FEATURE_MEDIA_ASSETS.brand.paypalIconSvg,
  );
            ...
            <IconLeadingButton
              ...
              leadingIcon={
                <img
                  src={githubLogoSrc}
                  alt="GitHub"
                  className="h-5 w-5 shrink-0 brightness-0 dark:invert"
                />
              }
