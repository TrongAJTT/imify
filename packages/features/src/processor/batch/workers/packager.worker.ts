...
import type { PackagerAddMessage, PackagerCancelMessage, PackagerFinalizeMessage, PackagerStartMessage, PackagerWorkerRequestMessage, PackagerWorkerResponseMessage } from "./packager-worker-protocol"

interface BasePackagerJob { mode: "zip" | "merge_pdf" | "pdf_zip"; totalFiles: number; receivedFiles: number; exportFileName: string; queue: Promise<void>; failed: boolean }
...
const workerPostMessage = globalThis as unknown as { postMessage: (message: PackagerWorkerResponseMessage, transfer?: Transferable[]) => void }
const jobs = new Map<number, PackagerJob>()
function postProgress(id: number, percent: number, message: string): void { workerPostMessage.postMessage({ id, type: "progress", percent, message }) }
function failJob(id: number, error: unknown): void {
  const job = jobs.get(id)
  if (job) { job.failed = true; jobs.delete(id) }
  workerPostMessage.postMessage({ id, type: "result", ok: false, error: error instanceof Error ? error.message : "Unknown packager worker error" })
}
...
function buildZipBytes(entries: ZipEntry[], level: ZipLevel): Uint8Array { const archive: Record<string, Uint8Array> = {}; for (const entry of entries) archive[entry.name] = entry.data; return zipSync(archive, { level }) }
function ensureJob(id: number): PackagerJob { const job = jobs.get(id); if (!job || job.failed) throw new Error("Packager job is missing or has already failed"); return job }
function enqueueJobStep(id: number, step: (job: PackagerJob) => Promise<void>): void {
  const job = ensureJob(id)
  job.queue = job.queue.then(async () => { const current = ensureJob(id); await step(current) }).catch((error) => { failJob(id, error) })
}
function onStart(message: PackagerStartMessage): void {
  if (jobs.get(message.id)) { failJob(message.id, new Error("Packager job id already exists")); return }
  const totalFiles = Math.max(0, Math.floor(message.totalFiles || 0))
  if (message.mode === "zip") { jobs.set(message.id, { mode: "zip", totalFiles, receivedFiles: 0, exportFileName: message.exportFileName, queue: Promise.resolve(), failed: false, entries: [], entryNames: new Set<string>(), zipLevel: parseZipLevel(message.zipLevel) }); postProgress(message.id, 6, "Collecting converted files..."); return }
  if (message.mode === "merge_pdf") { jobs.set(message.id, { mode: "merge_pdf", totalFiles, receivedFiles: 0, exportFileName: message.exportFileName, queue: Promise.resolve(), failed: false, blobs: [] }); postProgress(message.id, 8, "Collecting pages for merged PDF..."); return }
  jobs.set(message.id, { mode: "pdf_zip", totalFiles, receivedFiles: 0, exportFileName: message.exportFileName, queue: Promise.resolve(), failed: false, entries: [], entryNames: new Set<string>(), zipLevel: parseZipLevel(message.zipLevel) })
...
async function onAdd(message: PackagerAddMessage): Promise<void> {
  ...
  if (job.mode === "zip") {
    ...
    job.receivedFiles += 1
    postProgress(message.id, job.totalFiles > 0 ? Math.min(70, 10 + Math.round((job.receivedFiles / job.totalFiles) * 60)) : 70, `Added ${job.receivedFiles}/${job.totalFiles} files to ZIP...`)
    return
  }
  if (job.mode === "merge_pdf") {
    job.blobs.push(blob); job.receivedFiles += 1
...
async function onFinalize(message: PackagerFinalizeMessage): Promise<void> {
  ...
  if (job.mode === "zip") {
    ...
    workerPostMessage.postMessage({ id: message.id, type: "result", ok: true, outputBuffer, mimeType: "application/zip", outputFileName: job.exportFileName }, [outputBuffer]); return
  }
  if (job.mode === "merge_pdf") {
    postProgress(message.id, 64, "Building merged PDF file...")
    const outputBuffer = await (await mergeImagesToPdf(job.blobs)).arrayBuffer()
    jobs.delete(message.id)
    workerPostMessage.postMessage({ id: message.id, type: "result", ok: true, outputBuffer, mimeType: "application/pdf", outputFileName: job.exportFileName }, [outputBuffer]); return
  }
  postProgress(message.id, 88, "Compressing PDF ZIP archive...")
  const outputBuffer = toTransferableArrayBuffer(buildZipBytes(job.entries, job.zipLevel))
  jobs.delete(message.id)
  workerPostMessage.postMessage({ id: message.id, type: "result", ok: true, outputBuffer, mimeType: "application/zip", outputFileName: job.exportFileName }, [outputBuffer])
}
function onCancel(message: PackagerCancelMessage): void { const job = jobs.get(message.id); if (!job) return; job.failed = true; jobs.delete(message.id) }
self.onmessage = (event: MessageEvent<PackagerWorkerRequestMessage>) => {
  const message = event.data
  if (!message) return
  try {
    if (message.type === "start") { onStart(message); return }
    if (message.type === "cancel") { onCancel(message); return }
    if (message.type === "add") { enqueueJobStep(message.id, (job) => (job.failed ? Promise.resolve() : onAdd(message))); return }
