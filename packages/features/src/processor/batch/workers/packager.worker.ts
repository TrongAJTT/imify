...
const workerPostMessage = globalThis as unknown as {
  postMessage: (
    message: PackagerWorkerResponseMessage,
...
function failJob(id: number, error: unknown): void {
  ...
  workerPostMessage.postMessage({
    ...
    ok: false,
    error:
      error instanceof Error ? error.message : "Unknown packager worker error",
  });
}
...
function enqueueJobStep(
  id: number,
  step: (job: PackagerJob) => Promise<void>,
): void {
...
async function onAdd(message: PackagerAddMessage): Promise<void> {
  ...
  if (job.mode === "zip") {
    job.entries.push({
      name: ensureUniqueEntryName(message.name, job.entryNames),
      data: sourceBytes,
    });
    job.receivedFiles += 1;
    postProgress(
      ...
      job.totalFiles > 0
        ? Math.min(
            70,
            10 + Math.round((job.receivedFiles / job.totalFiles) * 60),
          )
        : 70,
  ...
  if (job.mode === "merge_pdf") {
    ...
    postProgress(
      ...
      job.totalFiles > 0
        ? Math.min(
            52,
            10 + Math.round((job.receivedFiles / job.totalFiles) * 42),
          )
        : 52,
  ...
  job.receivedFiles += 1;
  job.entries.push({
    name: ensureUniqueEntryName(
      `${sanitizeBaseName(message.name)}.pdf`,
      job.entryNames,
    ),
...
async function onFinalize(message: PackagerFinalizeMessage): Promise<void> {
  ...
  if (job.mode === "zip") {
    ...
    const outputBuffer = toTransferableArrayBuffer(
      buildZipBytes(job.entries, job.zipLevel),
    );
  ...
  if (job.mode === "merge_pdf") {
    ...
    const outputBuffer = await (
      await mergeImagesToPdf(job.blobs)
    ).arrayBuffer();
    jobs.delete(message.id);
    workerPostMessage.postMessage(
      {
  ...
  try {
    ...
    if (message.type === "add") {
      enqueueJobStep(message.id, (job) =>
        job.failed ? Promise.resolve() : onAdd(message),
      );
      return;
    }
    enqueueJobStep(message.id, (job) =>
      job.failed ? Promise.resolve() : onFinalize(message),
    );
  } catch (error) {
