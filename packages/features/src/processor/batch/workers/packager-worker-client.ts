  ...
  async run(params: PackagerRunParams): Promise<PackagerRunResult> {
    ...
    if (params.mode === "pdf_zip") {
      ...
      for (let i = 0; i < params.entries.length; i += 1) {
        ...
        const name = toUniqueName(
          `${sanitizeBaseName(entry.name)}.pdf`,
          usedNames,
        );
        archive[name] = new Uint8Array(await pdfBlob.arrayBuffer());
        report(
          Math.min(76, 10 + Math.round(((i + 1) / total) * 66)),
          `Created ${i + 1}/${total} PDF files...`,
        );
        await nextTick();
      }
      ...
      return {
        outputBlob: new Blob([bytes as BlobPart], { type: "application/zip" }),
        outputFileName: params.exportFileName,
      };
    }
    ...
    for (let i = 0; i < params.entries.length; i += 1) {
      ...
      report(
        Math.min(70, 10 + Math.round(((i + 1) / total) * 60)),
        `Added ${i + 1}/${total} files to ZIP...`,
      );
      await nextTick();
    }
