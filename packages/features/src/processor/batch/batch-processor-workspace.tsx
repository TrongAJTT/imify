...
export function BatchProcessorWorkspace() {
  ...
  const resizeContainBackground = useBatchStore(
    (s) => s.resizeContainBackground,
  );
  const resizeResamplingAlgorithm = useBatchStore(
    (s) => s.resizeResamplingAlgorithm,
  );
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      if (
        pdfSplitRef.current &&
        !pdfSplitRef.current.contains(event.target as Node)
      )
  ...
  const outputTotalAfterRun = useMemo(
    () =>
      successfulOutputs.reduce(
        (sum, item) => sum + (item.outputBlob as Blob).size,
        0,
      ),
    [successfulOutputs],
  );
  const reductionPercent = useMemo(() => {
    if (!sourceTotalAfterRun) return 0;
    const ratio =
      ((sourceTotalAfterRun - outputTotalAfterRun) / sourceTotalAfterRun) * 100;
    return Number.isFinite(ratio) ? ratio : 0;
  }, [sourceTotalAfterRun, outputTotalAfterRun]);
  const canRetryFailed = queueStats.error > 0 && !isRunning;
  const canRunAll =
    Boolean(effectiveConfig) && !isRunning && queueStats.queued > 0;
  ...
  return (
    <div className="p-0">
      ...
      {paused ? (
        <BodyText className="mt-3 text-indigo-700 dark:text-indigo-400">
          Batch is paused. Click Resume to continue.
        </BodyText>
      ) : null}
      {queueTooLarge ? (
        <BodyText className="mt-3 text-amber-700 dark:text-amber-400">
          Warning: queue size is {toMb(totalQueueBytes)} MB. You may hit memory
      ...
      {summary && !isRunning && queue.length > 0 ? (
        <BatchSummaryCard
          ...
          onTogglePdfSplit={() => setIsPdfSplitOpen((current) => !current)}
