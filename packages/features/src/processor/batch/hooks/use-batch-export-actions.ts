...
export function useBatchExportActions({
  ...
  const [activeExportAction, setActiveExportAction] =
  ...
  const clearExportToastHideTimer = () => {
    if (!exportToastHideTimerRef.current) return;
  ...
  const clearExportToast = (toastId?: string) => {
    ...
    setExportToastPayload((current) =>
      !current || (toastId && current.id !== toastId) ? current : null,
    );
  };
    ...
    try {
      for (const item of successful) {
        await downloadWithFilename(
          item.outputBlob as Blob,
          item.outputFileName as string,
        );
        await sleep(120);
      }
    } finally {
  ...
  const mergeIntoPdf = async () => {
    const successful = queue.filter(
      (item) => item.status === "success" && item.outputBlob,
    );
    if (!successful.length || isExporting) return;
    setIsExporting(true);
  ...
  const downloadIndividualPdfs = async () => {
    ...
    try {
      ...
      pushExportToast({
        ...
        status: "error",
        percent: 100,
        message: toUserFacingConversionError(
          error,
          "Unable to export individual PDFs",
        ),
      });
    } finally {
      setIsExporting(false);
