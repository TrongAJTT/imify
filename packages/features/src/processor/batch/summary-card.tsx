...
export function BatchSummaryCard(props: BatchSummaryCardProps) {
  ...
  const supportsPdfExport = ["jpg", "png", "webp", "bmp"].includes(
  ...
  return (
    <div className="rounded-xl border border-emerald-200 dark:border-emerald-800/50 bg-emerald-50/50 dark:bg-emerald-900/10 p-5 shadow-sm">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <Heading className="text-lg leading-tight">{t("summaryCard.batchCompleted")}</Heading>
          <MutedText className="mt-0.5">
            {t("summaryCard.processedStats", {
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-xl font-bold text-slate-800 dark:text-slate-100">
                  {formatBytes(sourceTotalAfterRun)}
                </span>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                ...
                {activeExportAction === "zip"
                  ? t("summaryCard.preparingZip")
                  : t("summaryCard.downloadZip", { count: successfulCount })}
              </Button>
              <Button
                variant="secondary"
                disabled={isExporting}
              ...
              >
                ...
                {activeExportAction === "one_by_one"
                  ? t("summaryCard.exportingFiles")
                  : t("summaryCard.oneByOne")}
              </Button>
              {supportsPdfExport ? (
                <div className="relative" ref={pdfSplitRef}>
                  <div className="inline-flex shadow-sm">
                    ...
                    >
                      ...
                      {activeExportAction === "merge_pdf"
                        ? t("summaryCard.mergingPdf")
                        : t("summaryCard.mergeSinglePdf")}
                    </Button>
                    <Button
                      variant="secondary"
                      className="rounded-l-none px-2.5"
                      aria-label={t("summaryCard.openPdfOptions")}
                      disabled={isExporting}
                    ...
                    <div className="absolute right-0 z-10 mt-2 min-w-[220px] rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg p-1.5 flex flex-col">
                      ...
                      >
                        {activeExportAction === "individual_pdf"
                          ? t("summaryCard.preparingIndividualPdfs")
                          : t("summaryCard.individualPdf", { count: successfulCount })}
                      </Button>
                    </div>
                  ) : null}
                </div>
