...
export function BatchQueueGrid({
  queue,
  isRunning,
  onRemoveItem,
}: BatchQueueGridProps) {
  ...
  return (
    ...
    >
      <div className="mt-6 grid gap-3 grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        {queue.map((item) => (
        ...
        {queue.length === 0 ? (
          <div className="col-span-full py-12 flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/10 text-slate-500 dark:text-slate-400">
            ...
            />
            <BodyText className="font-medium">{t("noFilesInQueue")}</BodyText>
          </div>
        ) : null}
      </div>
    </SortableContext>
