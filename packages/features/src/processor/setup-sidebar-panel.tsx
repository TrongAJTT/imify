...
export function BatchSetupSidebarPanel({
  ...
  const isTargetFormatQualityOpen = useBatchStore(
    (state) => state.isTargetFormatQualityOpen,
  );
  const isResizeOpen = useBatchStore((state) => state.isResizeOpen);
  const setIsTargetFormatQualityOpen = useBatchStore(
    (state) => state.setIsTargetFormatQualityOpen,
  );
  ...
  const watermark = useWatermarkStore(
    (state) => state.contextWatermarks[setupContext],
  );
  const watermarkSavedId = useWatermarkStore(
    (state) => state.contextSavedIds[setupContext] ?? null,
  );
  ...
  const onJxlProgressiveChange = useBatchStore(
    (state) => state.setJxlProgressive,
  );
  ...
  const onWebpNearLosslessChange = useBatchStore(
    (state) => state.setWebpNearLossless,
  );
  ...
  const onWebpPreserveExactAlphaChange = useBatchStore(
    (state) => state.setWebpPreserveExactAlpha,
  );
  const onAvifSpeedChange = useBatchStore((state) => state.setAvifSpeed);
  const onAvifQualityAlphaChange = useBatchStore(
    (state) => state.setAvifQualityAlpha,
  );
  const onAvifLosslessChange = useBatchStore((state) => state.setAvifLossless);
  const onAvifSubsampleChange = useBatchStore(
    (state) => state.setAvifSubsample,
  );
  const onAvifTuneChange = useBatchStore((state) => state.setAvifTune);
  const onAvifHighAlphaQualityChange = useBatchStore(
    (state) => state.setAvifHighAlphaQuality,
  );
  const onMozJpegProgressiveChange = useBatchStore(
    (state) => state.setMozJpegProgressive,
  );
  const onMozJpegChromaSubsamplingChange = useBatchStore(
    (state) => state.setMozJpegChromaSubsampling,
  );
  const onIcoSizesChange = useBatchStore((state) => state.setIcoSizes);
  const onIcoGenerateWebIconKitChange = useBatchStore(
    (state) => state.setIcoGenerateWebIconKit,
  );
  const onIcoOptimizeInternalPngLayersChange = useBatchStore(
    (state) => state.setIcoOptimizeInternalPngLayers,
  );
  ...
  const onResizeApplyToChange = useBatchStore(
    (state) => state.setResizeApplyTo,
  );
  ...
  const onResizeAspectModeChange = useBatchStore(
    (state) => state.setResizeAspectMode,
  );
  const onResizeAspectRatioChange = useBatchStore(
    (state) => state.setResizeAspectRatio,
  );
  const onResizeAnchorChange = useBatchStore((state) => state.setResizeAnchor);
  const onResizeFitModeChange = useBatchStore(
    (state) => state.setResizeFitMode,
  );
  const onResizeContainBackgroundChange = useBatchStore(
    (state) => state.setResizeContainBackground,
  );
  const onResizeResamplingAlgorithmChange = useBatchStore(
    (state) => state.setResizeResamplingAlgorithm,
  );
  ...
  const onPngCleanTransparentPixelsChange = useBatchStore(
    (state) => state.setPngCleanTransparentPixels,
  );
  const onPngAutoGrayscaleChange = useBatchStore(
    (state) => state.setPngAutoGrayscale,
  );
  const onPngDitheringLevelChange = useBatchStore(
    (state) => state.setPngDitheringLevel,
  );
  const onPngProgressiveInterlacedChange = useBatchStore(
    (state) => state.setPngProgressiveInterlaced,
  );
  const onPngOxiPngCompressionChange = useBatchStore(
    (state) => state.setPngOxiPngCompression,
  );
  const onBmpColorDepthChange = useBatchStore(
    (state) => state.setBmpColorDepth,
  );
  const onBmpDitheringLevelChange = useBatchStore(
    (state) => state.setBmpDitheringLevel,
  );
  const onTiffColorModeChange = useBatchStore(
    (state) => state.setTiffColorMode,
  );
  const onFileNamePatternChange = useBatchStore(
    (state) => state.setFileNamePattern,
  );
  const setContextWatermark = useWatermarkStore(
    (state) => state.setContextWatermark,
  );
  ...
  const supportsExif = ["jpg", "webp", "avif", "mozjpeg"].includes(
    targetFormat,
  );
  const isIcoTarget = targetFormat === "ico";
  const formatAdvancedLabel = useMemo(
    () => getFormatAdvancedLabel(targetFormat, t),
    [targetFormat, t],
  );
  ...
  const sidebarItems: WorkspaceConfigSidebarItem[] = [
    {
      ...
      content: (
        <TargetFormatQualityCard
          ...
          onTargetFormatChange={(nextValue: string) =>
            onTargetFormatChange(nextValue as BatchTargetFormat)
          }
    ...
    },
    {
      ...
      content: (
        <ResizeCard
          ...
          onResizeAspectModeChange={(mode) =>
            onResizeAspectModeChange(mode as any)
          }
          onResizeAspectRatioChange={(ratio) =>
            onResizeAspectRatioChange(String(ratio))
          }
