...
export function ImageUrlImportControl({ allowMultiple, onProcessUrls, disabled = false, className }: ImageUrlImportControlProps) {
  ...
  const processLabel = allowMultiple ? t("importUrls") : t("importFirstUrl")
  ...
  const handleProcess = async () => {
    ...
    if (!urlsToImport.length) { setErrorText(t("enterValidUrlError")); return }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-xl border border-slate-200 bg-white p-4 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">{t("importImageUrlTitle")}</h3>
                <MutedText className="mt-1 text-xs">{allowMultiple ? t("importImageUrlSubtitleMultiple") : t("importImageUrlSubtitleSingle")}</MutedText>
              </div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
              <span>{t("lines")}: {nonEmptyLineCount}</span><span>{t("validUrls")}: {parsedUrls.length}</span>
              {!allowMultiple && parsedUrls.length > 1 ? <span className="rounded bg-amber-100 px-2 py-0.5 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300">{t("onlyFirstUrlWarning")}</span> : null}
            </div>
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              <SecondaryButton disabled={isSubmitting} onClick={handleClose}>{t("cancel")}</SecondaryButton>
