  ...
  return (
    <div className="relative group">
      ...
      >
        {icon ? (
          <div
            className={`shrink-0 flex items-center justify-center ${themeClasses.icon}`}
          >
            {icon}
          </div>
        ) : null}
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate text-[12px] font-bold text-slate-700 dark:text-slate-300">
            {label}
          </div>
          {sublabel ? (
            <div className="whitespace-pre-wrap break-words leading-tight text-[10px] text-slate-400">
              {sublabel}
            </div>
          ) : null}
        </div>
        ...
        {(tooltipContent || tooltipLabel) && (
          <div className="shrink-0 ml-1">
            ...
            >
              <HelpCircle
                size={14}
                className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-help transition-colors"
              />
            </Tooltip>
          </div>
        )}
