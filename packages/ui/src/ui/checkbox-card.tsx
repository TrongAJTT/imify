  ...
  return (
    ...
    >
      <div className="flex items-start gap-2.5 min-w-0 flex-1">
        {icon && (
          <div className={`shrink-0 m-auto ${themeClasses.icon}`}>{icon}</div>
        )}
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="font-bold text-[12px] text-slate-700 dark:text-slate-300 whitespace-nowrap truncate">
              {title}
            </span>
          </div>
          {subtitle && (
            <div className="text-[10px] mt-0.5 opacity-70 w-full leading-tight whitespace-pre-wrap break-words">
              {subtitle}
            </div>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 shrink-0 ml-2">
        ...
        <input
          ...
          type="checkbox"
          className={`rounded border-slate-300 focus:ring-2 focus:ring-offset-0 sr-only ${
            isSky
