...
import {
  ReorderMenuPopover,
  type ReorderMenuPopoverItem,
} from "./reorder-menu-popover";
...
function resolveWorkspaceConfigItemLabel(
  item: WorkspaceConfigSidebarItem,
): string {
...
export function WorkspaceConfigSidebarPanel({
  ...
  const activeMenuIndex = reorderMenu
    ? orderedIds.indexOf(reorderMenu.itemId)
    : -1;
  const canMoveUp = activeMenuIndex > 0;
  const canMoveDown =
    activeMenuIndex >= 0 && activeMenuIndex < orderedIds.length - 1;
