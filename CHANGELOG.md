...
# Update v2.3.3 - 2026-08-28

### Added

- **Continuous Collage Layout Browsing**:
  - Browse all collage templates from 2 to 10 photos in a single, seamless scrolling list.
  - Reorder both built-in and custom collage layouts via intuitive drag-and-drop.
  - Rename custom collage presets directly on their layout cards.
- **Interactive Canvas Reordering in Grid Designer**:
  - Direct canvas drag-and-drop handles to visually reorder rows and columns in real-time.
  - Spatial drop snapping and merged group handles for complex non-convex cell arrangements.
  - Dedicated Primary Direction toolbar toggle and cell sequence reversal.
  - Save custom grid designs directly as reusable Collage Presets or browse and apply existing templates with a single click.
- **Collapsible Accordion Banners in Asset Management**:
  - Modernized info banners across AI Models, Fonts, and Collage Presets tabs with collapsible `Accordion Card`.
- **Enhanced Multiline Captions in Image Splicing**:
  - Full multiline text caption support with `Ctrl+Enter` keyboard shortcut to quickly confirm.
  - Unified rounded corner radius styling for outside captions.
- **Resize Quick Stats**:
  - Added Shortest Edge and Longest Edge dimension metrics to the Quick Stats panel.

### Changed

- **Collage Maker Input & Shortcuts**:
  - Added direct clipboard paste (`Ctrl+V`) and drag-and-drop image intake support in Stage 1.
  - Added quick shortcut actions in empty states and toolbars.

### Fixed

- **QR Reader Toast Notifications**: Eliminated redundant notifications when activating camera or screen capture, showing alerts strictly for successful decodings or genuine scan errors.
- **Mobile Dialog Scrolling**: Fixed viewport scroll clipping and touch container heights on mobile screens.
