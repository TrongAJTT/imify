...
function useDiffcheckerStoreHydrated(): boolean {
  ...
  useEffect(() => {
    ...
    const unsubStart = useDiffcheckerStore.persist.onHydrate(() =>
      setHydrated(false),
    );
    const unsubFinish = useDiffcheckerStore.persist.onFinishHydration(() =>
      setHydrated(true),
    );
    return () => {
      unsubStart();
...
export function DiffcheckerPage() {
  ...
  useEffect(() => {
    ...
    setHeaderBreadcrumb(<FeatureBreadcrumb compact rootToolId="diffchecker" />);
    return () => resetHeader();
