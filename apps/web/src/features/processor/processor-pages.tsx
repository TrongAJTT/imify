...
function getContextToolId(
  context: SetupContext,
): "single-processor" | "batch-processor" {
  return context === "single" ? "single-processor" : "batch-processor";
}
...
export function ProcessorLandingPage({ context }: ProcessorLandingPageProps) {
  ...
  const ensureDefaultPresetForContext = useBatchStore(
    (state) => state.ensureDefaultPresetForContext,
  );
  const isBatchStoreRehydrated = useBatchStore(
    (store) => (store as any)._hasHydrated,
  );
  const scopedPresets = presets;
  const sidebar = useMemo(
    () => <ProcessorSidebarShell context={context} workspaceSidebar={null} />,
    [context],
  );
...
export function ProcessorWorkPage({
  context,
  presetId,
}: ProcessorWorkPageProps) {
  const { t } = useTranslation("processor");
  ...
  const setSetupContext = useBatchStore((state) => state.setSetupContext);
  const presets = useBatchStore((state) => state.presets);
  const activePresetId = useBatchStore(
    (state) => state.activePresetIds[context] ?? null,
  );
  ...
  const applyPresetToCurrentContext = useBatchStore(
    (state) => state.applyPresetToCurrentContext,
  );
  const setPresetViewMode = useBatchStore((state) => state.setPresetViewMode);
  const syncActivePresetConfig = useBatchStore(
  ...
  useEffect(() => {
    ...
    const hasActivePreset =
      !!activePresetId &&
      scopedPresets.some((entry) => entry.id === activePresetId);
    if (hasActivePreset) {
      return;
    }

    const fallbackPresetId =
      recentPresetIds[context] ?? scopedPresets[0]?.id ?? null;
    if (fallbackPresetId) {
  ...
  if (!preset) {
    return (
      <WorkspaceNotFoundState
        title={t("presetNotFound")}
        message={t("presetNotFoundMessage", {
