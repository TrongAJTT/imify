...
import { ChevronDown, ChevronUp, Eye, Sparkles, Telescope } from "lucide-react"
...
export function SeoAuditSnapshotInline({ report }: { report: SeoAuditReport }) {
  ...
  const sortedAssets = [...report.assets].sort(
    (left, right) =>
      (right.estimatedSavingsBytes ?? 0) - (left.estimatedSavingsBytes ?? 0)
  )
  const topRecommendations =
    report.recommendations.length > 0
      ? report.recommendations.slice(0, 3)
      : ["No critical SEO image issue detected."]
    ...
    <div className="space-y-2">
      <section className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between gap-2">
          <div>
            <Subheading className="text-base leading-5">
              SEO Audit Snapshot
            </Subheading>
          </div>
          <span
            className={`rounded-md text-center px-2 py-1 text-[10px] font-semibold ${levelClass(report.summary.level)}`}>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5">
            <Subheading className="text-base leading-5">
              Assets to review ({sortedAssets.length})
            </Subheading>
            <Tooltip
          ...
          <button
            ...
            aria-label={
              isAssetsCollapsed ? "Expand assets list" : "Collapse assets list"
            }
        ...
        <div
          className={`mt-2 ${isAssetsCollapsed ? "max-h-[24rem] overflow-y-auto pr-1" : ""}`}>
              ...
              <div
                ...
                <div className="relative aspect-[4/3] w-full overflow-hidden bg-slate-100 dark:bg-slate-800">
                  <div className="absolute right-2 top-2 z-10 flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
                    <Tooltip
                      content={SEO_AUDIT_TOOLTIPS.inspectAction}
                      variant="nowrap">
                      <button
                        type="button"
                    ...
                    <Tooltip
                      content={SEO_AUDIT_TOOLTIPS.optimizeAction}
                      variant="nowrap">
                      <button
                        type="button"
                ...
                <div className="px-2 py-1.5">
                  <BodyText className="truncate text-[11px] font-medium text-slate-800 dark:text-slate-100">
                    {asset.url}
                  </BodyText>
                  <MutedText className="mt-0.5 text-[11px]">
                    {asset.source} • save{" "}
                    {formatBytes(asset.estimatedSavingsBytes ?? 0)}
                  </MutedText>
                </div>
              </div>
            ))}
