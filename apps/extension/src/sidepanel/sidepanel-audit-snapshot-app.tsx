    ...
    try {
      ...
      setActionError(
        error instanceof Error
          ? error.message
          : t("sidepanel.scanFailedError", "Failed to run SEO audit scan.")
      )
    } finally {
      setIsScanning(false)
    }
      ...
      <div className="space-y-3">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <Kicker className="text-[11px]">
            {t("sidepanel.scanStatus", "Scan status")}
          </Kicker>
          <BodyText className="mt-1 font-medium text-slate-800 dark:text-slate-100">
            {statusText}
          </BodyText>
          <div className="mt-2 flex items-center gap-2">
            <Button
              ...
              {isScanning
                ? t("sidepanel.scanning", "Scanning...")
                : t("sidepanel.scanNow", "Scan now")}
            </Button>
            <Tooltip
              content={t("sidepanel.lastScanned", "Last scanned: {{time}}", {
                time: scanTimeLabel
              })}>
        ...
        {actionError ? (
          <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-700 shadow-sm dark:border-red-900/50 dark:bg-red-900/20 dark:text-red-300">
            <BodyText className="text-xs text-red-700 dark:text-red-300">
              {actionError}
            </BodyText>
          </div>
        ) : null}
