...
export function ImageUrlImportControl({
  allowMultiple,
  onProcessUrls,
  disabled = false,
  className
}: ImageUrlImportControlProps) {
  ...
  const handleProcess = async () => {
    ...
    if (!urlsToImport.length) {
      setErrorText("Enter at least one valid http(s) image URL.")
      return
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-xl border border-slate-200 bg-white p-4 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                ...
                <MutedText className="mt-1 text-xs">
                  {allowMultiple
                    ? "Paste one URL per line. All valid image URLs will be imported."
                    : "Paste one or more URLs. The first valid URL will be imported."}
                </MutedText>
              </div>
              <button
                type="button"
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
              <span>Lines: {nonEmptyLineCount}</span>
              <span>Valid URLs: {parsedUrls.length}</span>
              {!allowMultiple && parsedUrls.length > 1 ? (
                <span className="rounded bg-amber-100 px-2 py-0.5 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300">
                  Only first URL will be used in Single Processor
                </span>
              ) : null}
            </div>
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              ...
              <Button type="button" variant="primary" disabled={isSubmitting} onClick={handleProcess}>
